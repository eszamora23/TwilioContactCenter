...
export default function AgentApp() {
  ...
  const sections = [
    { id: 'softphone', label: 'Softphone', content: () => <Softphone />, view: <Softphone /> },
    { id: 'presence', label: 'Presence', content: () => <Presence />, view: <Presence /> },
    {
      id: 'customer360',
      label: 'Customer 360',
      content: () => <Customer360 />,
    ...
    },
    {
      ...
      view: <TasksPanel setAvailable={setAvailable} />,
    },
    {
      id: 'reservations',
      label: 'Reservations',
  ...
  return (
    <Box minHeight="100vh" width="100%">
      ...
      <AgentDesktopShell sections={sections} title="Agent Desktop" actions={headerActions}>
        <Box marginBottom="space70">
          <StatusBar label={activity || ''} onChange={(sid) => setAvailable(sid)} />
        </Box>

        <Box ref={scrollRootRef}>
          {sections.map(({ id, label, view }) => (
