...
function syncProfileToFields(){
  ...
  const defaultId = (DEMO.customerProfiles && DEMO.customerProfiles[0]?.id) || null;
  // Asegura “Alex Johnson” (primero) seleccionado
  if (profileSelect.options.length) {
    const want = defaultId || profileSelect.options[0].value;
    const idx = Array.from(profileSelect.options).findIndex(o => o.value === want);
    if (idx >= 0) profileSelect.selectedIndex = idx;
  }
  const p = (DEMO.customerProfiles || []).find(x => x.id === profileSelect.value);
  if(!p) return;
  if (callerId) callerId.value = p.callerId || '';
  if (ivrNumber) ivrNumber.value = DEMO.ivrNumber || '';
  if (scenario) scenario.value = p.scenario || '';
  if (dtmf) dtmf.value = p.dtmf || '';
  if (locale) locale.value = p.locale || 'en-US';
...
// Pinta lista de perfiles y vincula cambios
...
// Re-sincroniza al navegar por ancla (por si el usuario entra directo a #phone-lab)
window.addEventListener('hashchange', () => {
  if (location.hash === '#phone-lab') syncProfileToFields();
