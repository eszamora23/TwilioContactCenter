...
import * as trService from '../services/taskrouter.js';
...
export async function events(req, res) {
  ...
  if (req.body.EventType === 'worker.activity.update') {
    const presenceData = await trService.listWorkers();
...
export async function activities(_req, res) {
  try {
    const list = await trService.listActivities();
...
export async function myTasks(req, res) {
  try {
    ...
    const reservations = await trService.listWorkerReservations(workerSid);
    const uniqueTaskSids = [...new Set(reservations.map(r => r.taskSid).filter(Boolean))];
    const fetchedTasks = await Promise.all(
      uniqueTaskSids.map(taskSid => trService.fetchTask(taskSid).catch(() => null))
    );
...
export async function completeTask(req, res) {
  try {
    ...
    const task = await trService.fetchTask(taskSid);
...
export async function availableWorkers(_req, res) {
  try {
    const acts = await trService.listActivities();
    const availableSids = acts.filter(a => a.available).map(a => a.sid);
    const workers = await trService.listWorkers();
    const out = [];
...
export async function presence(_req, res) {
  try {
    const workers = await trService.listWorkersWithRetry(2, 500);
...
export function recentEvents(_req, res) {
  res.json(trService.getRecentEvents());
}
